...
/// </remarks>
public sealed class ParticleFlux : IParticle
{
    ...
    /// </summary>
    public ParticleFlux(EndPoint remoteEndPoint, int bufferSize = 8192, int maxDegreeOfParallelism = 8)
    ...
    /// </summary>
    public ParticleFlux(Socket connectedSocket, int bufferSize = 8192, int maxDegreeOfParallelism = 8)
    {
    ...
    public async ValueTask SendAsync(ReadOnlyMemory<byte> payload)
    {
        ...
        sendArgs.UserToken = new Particle.SendToken(tcs, sendArgs.MemoryBuffer);
    ...
    {
        var token = (Particle.SendToken)e.UserToken;
